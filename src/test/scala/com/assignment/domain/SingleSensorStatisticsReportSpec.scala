...
object SingleSensorStatisticsReportSpec extends DefaultRunnableSpec {
  ...
  def spec = suite("SingleSensorStatisticsReport")(
    testM("update should correctly update numOfFailedMeasurements") {
    ...
    testM("update should correctly update numOfProcessedMeasurements") {
      check(genReport, genMeasurement) { (report, measurement) =>
          assert(report.update(measurement).numOfProcessedMeasurements)(equalTo(report.numOfProcessedMeasurements + 1))
      }
    },
    testM("update should correctly update sensorStatistics when there is measured humidity and no statistics from sensor yet") {
    ...
    testM("update should correctly update sensorStatistics when there is no measured humidity") {
    ...
    ,
    test("update should correctly update sensorStatistics max value") {
    ...
    test("update should correctly update sensorStatistics min value") {
      val report = SingleSensorStatisticsReport("sensorId", 1, 0, Some(SensorStatistics(1, 2, 3)))
      val measurement = SensorMeasurement("sensorId", Some(0))

      assert(report.update(measurement).sensorStatistics.get.min)(equalTo(0))
    },
    test("update should correctly update sensorStatistics avg value") {
